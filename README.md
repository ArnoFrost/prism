...
当前阶段口径：`v2.0.0`（v1.0/v1.1.x 是历史里程碑）。v2.0 主线四段叙事——历史包袱清偿 / `prism pipeline` deprecated alias 物理移除 / 治理路径默认弱化（workflow + 痕迹义务家族明确为可选项）/ workflow 复杂度简化——已完成并进入 GA 口径；当前重点转向 v2.1 的代码流程入口治理与默认面降噪。
