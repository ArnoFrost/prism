...
[![Stage](https://img.shields.io/badge/stage-4.0.0-canary.6-release-blue)](CHANGELOG.md)
...
**当前发行**：4.0.0-canary.6。当前使用 **experimental natural dogfood** Distribution Profile，只分发 `/prism`、`/prism-review`、`/prism-plan` 三个入口。3.x 实现已从本分支剔除，终态见 git tag `legacy-3x-final`；旧 topic 在本分支只读。
...
cd ~/prism
git switch --detach v4.0.0-canary.6
./setup.sh init
...
> 帮我 clone `https://github.com/ArnoFrost/prism.git` 到 `~/prism`，切换到 detached Tag `v4.0.0-canary.6`，执行 `./setup.sh init` 使用默认本地 Workspace backend，再运行 `prism update canary --no-fetch` 选择测试通道，并用 `prism --version` 与 `./setup.sh check` 完成验收。
...
| 项 | 当前口径 |
|----|----------|
| Release | [`4.0.0-canary.6`](VERSION) |
| Package version | `4.0.0.dev6`（PEP 440） |
| Python | 3.11+ |
| License | [MIT](LICENSE) |
| 当前命令面 | `prism --help` |
