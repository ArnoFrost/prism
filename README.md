...
> **workflow / 痕迹义务家族都是可选项**（029/r07 AP-46）
>
> Prism 的 core contract（最小运行合同）只要求 SDK + Vault Workspace + `uv`。**workflow 系列技能** 与配套的 **痕迹义务家族**（`task_probe` / `decision_artifact` / `intake_gate_out` / `merge_artifact`）是治理框架的可选增强：
>
...
> - **治理类 topic 例外（`029_*` 默认 strict）**：专项目录名以 `029_` 开头的为治理 dogfooding 主战场，Step 2.5 默认升级为 strict（任一族 missing 即 ERR）；可被 frontmatter `trace_strict: false` / `PRISM_TRACE_VALIDATE=lenient` / `--trace-lenient` 显式覆盖。完整优先级：CLI flag > ENV > frontmatter > `029_*` 默认 strict > 全局默认 lenient（见 `prism_cli._resolve_trace_strict`）
>
