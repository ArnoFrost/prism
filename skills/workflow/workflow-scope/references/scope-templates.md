...
| **semantic** | Delta/用户意图含 task 拆分、升格 structures、新建 task-N（**FS-semantic-fork**）| **不可跳过** task-fork gate 三选一（不依赖 SR） |
| **orphan-index** | `task.index` 有数据行但 `task_count=0`（**SIG-INDEX-ORPHAN**）| advisory + handoff workflow-scope；validate 默认 WARN |
...
本轮写盘：{scope 原地 | +task-N 骨架（含 scope.md，可占位 wave）+ task.index | 仅计划不动盘}
```

**Agent 规则**：

1. `require_fork_gate` 或 **FS-semantic-fork**（见 SKILL.md）却省略本块 → FS-skip-delta-fail。
2. 选「膨胀 task」→ Phase 3 完成 **Task Spawn Checklist**（下节）；**禁止**仅 `+task.index` 无 task-N 目录。
...
选「膨胀 task」时 Phase 3 **逐项勾选**（写盘顺序固定）：

- [ ] `structures/task-N_{slug}/scope.md` 首版（task-V 1:1 投影 topic-V；**先于** index）
- [ ] `structures/task.index.md` 新增一行（task-N + label + 投影 V + 授权来源）
- [ ] `task.index.md` §升级触发器 回填一条（为何 S3 满足）
- [ ] `focus.md` 保留区补 task-N / wave 链（wave 可下轮占位）
