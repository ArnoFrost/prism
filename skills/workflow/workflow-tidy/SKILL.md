...
name: workflow-tidy
description: |
  工件机械对齐 — review/decision 后同步 rXX decision 镜像、既有 review.index 与 frontmatter，并兜底刷新存量 README 指针（grandfather）；decision.index 由 dXX 维护，tidy 不 auto-fix。
  Use when: 工件对齐、状态同步、review 后收尾、workflow-tidy
description_zh: "工件机械对齐 — review/decision 后同步 rXX decision 镜像、既有 review.index 与 frontmatter；decision.index 由 dXX 维护。"
license: MIT
metadata:
  author: ArnoFrost
  version: 3.0.0
...
public_gate:
  ...
  rationale: "Prism 3.0 工件机械对齐入口，只修 how、不改变 scope 的 what。"
  rollback: "将 catalog 与 SKILL 镜像恢复为 dev/experimental；机械修复结果无需回滚。"
  ssot_id: workflow-tidy
---
...
| review/decision 落盘后 rXX decision 镜像、既有 review.index（及 grandfather README）指针过时 | `/workflow-tidy` |
...
| 修复既有 review.index.md 缺失行 | 调整优先级 |
...
| 既有 review.index.md 缺失条目 | decisions/*.md 的 `review_ref` → reviews/ | 是（文件缺失时跳过） |
| rXX `decision_status` / `decision_ref` | 最新完整 dXX 主链的精确 `review_ref` → reviews/ | 是 |
...
- **review.index.md**：可选导航索引，缺失合法。仅当文件已经存在时，缺失镜像行才可 `--fix` 补全
- **rXX Decision 镜像**：仅在 dXX 完整主链 + 精确 `review_ref` 后原子 `--fix`；裸 review 不推断，断链只报告并阻塞正式 finalize
- **decision.index.md**：**仅 report** — 由 Gate 4 / dXX 追加，tidy 不得自动写入

稀疏资格只影响既有 review.index 镜像：Accept/Reject/Defer 都会写 dXX，其 `review_ref` 可赋予镜像资格；Other 不写 dXX，磁盘上单独存在 review 不构成资格。Tidy 不得为了单次 Review 或 Decision 自动创建 `review.index.md`。
...
- **正常触发**：dXX 已引用 review 且旧 topic 已有 `review.index.md` 后「帮我对齐 review.index」→ 默认 dry-run 只报告 eligible 缺行，用户确认后 `--fix` 应用安全项；裸 review 不入索引，缺少 `review.index.md` 不创建。
...
- [ ] `--fix` 只应用安全项（既有 eligible review.index 补行、rXX decision 镜像、frontmatter updated、grandfather 指针、wikilink 残留）
