...
> - 用户回复必须严格匹配 §5 协议（数字 / Accept|accept|"accept it" / Reject|... / Defer|...）；
...
#### 4.3.3 mode 决策异常场景（用户未指定 + 自动判定不可信）

```
mode 自动判定失败（评审材料 / 文件数 / 并行能力均无法可靠估计）。

请确认 mode：
  [1] full — 多角色并行 + 深度审查（评审材料 > 200 行 或 涉及 3+ 文件 推荐）
  [2] quick — 单视角串行 + 快速扫描（小改动推荐）
  [3] 取消并升级到 /workflow-review-lite（更轻量入口）
