...
> **被引用方**：`workflow-intake/SKILL.md` Phase 2 路由门、`workflow-review/SKILL.md` Gate 4 决策门、`workflow-review-lite/SKILL.md` §4 决策门、`workflow-intake/references/intake-fallback.md`。
...
sniff 检测到本次 intake 与已有 topic 有亲和：
  ...
  候选 2：append 到 026_yyy（sniff 最高匹配，score=2，非默认）
  候选 3：append 到 027_zzz（score=2，非默认）

请回复你的选择（编号 / 描述 / "新建" / "append 到 #xx"）：
```
...
> - 解析失败 / 超时 / 用户取消时**禁止写入 `decisions/dXX.md`**（决策门错选会固化错误共识 + 串联 `prism legacy finalize`，回溯成本高 — r13 P0 F2）。
...
> [!warning]
> **触发前置（r13 PostFix）**：仅当 ① 材料路径不可达 ② 文件数无法枚举 ③ 并行能力探测失败超 1 次 **同时满足**才触发本节模板。**正常路径必须走 workflow-review/SKILL.md §1 自动判定，不允许"保险起见 Ask"**。
...
> [!IMPORTANT]
> **本节触发后 Agent 行为约束**：
> - 选 [1] 后**仍走 workflow-review/SKILL.md §策略一**（mode=full 必须真并行子任务），不得退化为"在 fallback 里我就串行了"；
...
> [!note]
> **本表是"门类 × 实例 → 频率档 × 模板取向"的 SSOT**（030/AP-74 r14 OQ-6）。与 [topic-sniff-spec.md §0.1](../topic-sniff-spec.md) 的 "skill × suggestion → 默认动作" 表互补不重叠：
> - §0.1 服务于 sniff 阶段（决定路由意图）
> - 本表 §7.1 服务于 fallback 阶段（决定模板严格度）
> - 任何 skill 频率分档讨论都应链回这两表之一，不得在 SKILL.md 中再造第三套

| Skill | 门类 | 实例 | 频率特征 | 模板取向 |
|---|---|---|---|---|
| **intake** | 路由门（§4.1） | Phase 2 路由决策（new/append/ask_user） | **低频启动**（每次 intake 1 次） | 候选清单庄重、默认新建；append 须显式目标，见 [intake-routing-spec.md](../../workflow-intake/references/intake-routing-spec.md) |
...
| 2026-05-14 | 030/AP-74 r14 OQ-6 sniff_as_ssot 收尾：§7.1 加 SSOT 边界标注（与 topic-sniff-spec §0.1 维度互补不重叠），关闭"两表是否同 SSOT"的疑虑（v2.0-canary）|
