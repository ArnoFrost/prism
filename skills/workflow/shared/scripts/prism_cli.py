...
# 内置默认 strict 前缀 — 测试可 monkeypatch 本模块属性；finalize 编排读此值
_STRICT_DEFAULT_PREFIXES: tuple[str, ...] = ()

from finalize_runner import run_finalize


def _resolve_trace_strict(topic_dir: str, cli_override: str | None) -> tuple[str, str]:
    from finalize_runner import resolve_trace_strict

    return resolve_trace_strict(topic_dir, cli_override, strict_prefixes=_STRICT_DEFAULT_PREFIXES)


from parse_utils import extract_frontmatter_field as _extract_frontmatter_field


def cmd_finalize(args: argparse.Namespace) -> int:
    """Decision 后一键编排：tidy --fix → validate --fix → validate-trace → validate-review-call → scope 提示。

    实现见 finalize_runner.run_finalize；步骤序与 trace 语义 SSOT 在该模块。
    """
