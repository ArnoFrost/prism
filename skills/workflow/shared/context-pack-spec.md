...
| 3 | `plan.md` | 当前焦点、待执行摘要（仅标题行）、已完成摘要 | 跳过，标记 `plan: null` |
...
| **scope** | light | 只需 scope + plan + 触发源（决策/review 由调用者传入） |
...
| `status/scripts/status.py` | 独立职责（全 workspace 扫描），不替换；但 light 模式的 scope/plan 提取逻辑可共享 |
