...
适用于：`review`、`intake`（`mode=append` 显式追加）
...
| 5 | `review.index.md` | 可选派生导航索引；缺失合法 | 跳过 |
...
| Skill | 默认档位 | 说明 |
|-------|---------|------|
| **intake** | full（append） / light（新建） | 默认新建 topic 无历史工件可读，自动降级 light；`--append` 读已有 topic 时用 full |
...
| `workflow-status/scripts/status.py` | 独立职责（全 workspace 扫描），不替换；但 light 模式的 scope/focus 提取逻辑可共享 |
