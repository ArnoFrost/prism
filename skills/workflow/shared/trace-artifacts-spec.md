...
intake_gate_out:
  ...
  focus_md_present: true | false           # focus.md 至少占位
...
- `scope_md_present` / `focus_md_present` / `readme_md_present` / `review_index_present` 任一为 `false` → **违约**：intake 必须补占位骨架；intake 完成前**禁止**进入下游 scope/review 阶段（2.x 兼容口径见 focus-derive-spec §2.x 兼容）
