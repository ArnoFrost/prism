...
| **读取工件** | topic 上下文按 [context-pack-spec](references/context-pack-spec.md) light 档装配（scope.md / focus.md；README 仅 grandfather 存量兜底）；另读最近决策/review 结论（由调用者传入） |
| **写入工件** | scope.md（原地更新）、focus.md（rewrite 刷新 — **入口面**）、decision.index.md（追加决策）、structures/task.index.md（若 topic 有 task）；README.md 仅当存量 topic 仍维护时同步 |
| **结束建议** | → `workflow-review` 或 `workflow-review-lite`（验证变更）；或回到执行 |
| **设计模式** | Pattern 1 — Sequential Workflow（读取→识别delta→更新scope→刷新focus→同步索引） + Pattern 5 — Domain-specific Intelligence（合同规则：scope 原地更新、focus 刷新链不可绕过） |
...
Phase 3  Update（更新 scope + 刷新 focus）
  ↓
Phase 4  Sync（刷新 focus 入口 + 索引；README 仅 grandfather 兜底）
```
...
读取专项根目录下的：
- `scope.md` — 当前合同
- `focus.md` — 当前工作集 + **入口（保留区双链）**
- `README.md` — 仅存量 grandfather topic 才有（deprecated，见 [topic-format-spec](../shared/topic-format-spec.md) §2）
...
- `focus.md`：保留区双链（scope/decision.index/review.index）是 topic **入口**，确保最新（rewrite 已在 Phase 3 完成）
- `decision.index.md`：如果本次触发了新决策，追加记录
- `README.md`：**deprecated** — 仅当存量 grandfather topic 仍维护 README 时，最小同步"当前状态/阶段"；新 topic 不再维护 README
...
| `focus.md` 保留区 | 双链入口 | topic **导航入口**（取代 README）|
| `README.md` | grandfather 兜底 | deprecated；仅存量 topic 最小同步 |
