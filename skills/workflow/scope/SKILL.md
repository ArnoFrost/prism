---
name: workflow-scope
description: "在决策后更新 scope.md 并派生 plan.md，确保合同面一致性。当接受评审决策或发现 scope 与执行偏移时触发。scope 是 plan 唯一上游。 Use when: 接受决策后同步、scope 偏移修正、边界收敛、plan 派生、workflow-scope"
visibility: dev
stability: experimental
description_zh: "在决策后更新 scope.md 并派生 plan.md，确保合同面一致性。当接受评审决策或发现 scope 与执行偏移时触发。scope 是 plan 唯一上游。"
---
## 职责边界
...
| **读取工件** | topic 上下文按 [context-pack-spec](references/context-pack-spec.md) light 档装配（scope.md / plan.md / README.md）；另读最近决策/review 结论（由调用者传入） |
...
> **术语**：本 SKILL 中 scope / plan / G / V / OQ / AP / phase / wave 等术语遵循 [vocabulary.md](references/vocabulary.md) — 首批 8 术语 + 形态类型 + 14 组易混淆对比 + Prefix dispatch 表见 SSOT；**不字字复制本体定义**。
...
按 [plan-derive-spec](references/plan-derive-spec.md) 执行 scope.md 更新 + plan.md 派生。
