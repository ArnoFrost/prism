...
name: workflow-scope
description: |
  在决策后更新 scope.md 并刷新 focus.md（3.0；2.x 存量 grandfather 派生 plan.md），确保合同面一致性。当接受评审决策或发现 scope 与执行偏移时触发。scope 是 focus 与 structures/task.index 的唯一上游。
...
| **读取工件** | topic 上下文按 [context-pack-spec](references/context-pack-spec.md) light 档装配（scope.md / focus.md / README.md；2.x grandfather 回退 plan.md）；另读最近决策/review 结论（由调用者传入） |
| **写入工件** | scope.md（原地更新）、focus.md（rewrite 刷新；2.x grandfather 派生 plan.md）、README.md（状态段更新）、structures/task.index.md（若 topic 有 task） |
...
读取专项根目录下的：
- `scope.md` — 当前合同
- `focus.md` — 当前工作集（2.x grandfather 回退 `plan.md`）
- `README.md` — 当前状态
...
> 规范详见 `shared/focus-derive-spec.md`，此处不重复内联规则。2.x 存量 topic（仍 `plan.md`）走 grandfather，见 `shared/plan-derive-spec.md`（deprecated）。
...
| `focus.md` | 从 scope rewrite 刷新 | 当前工作集，主体≤30行；retention=rewrite，不累积历史（2.x grandfather 派生 `plan.md`）|
