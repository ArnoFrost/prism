...
| C-RELEASE | tag grammar、通道隔离与发行 / 更新机械面：check 无副作用、push 需显式确认、更新只认不可变 tag | `bin/tag_resolve.py`、`bin/release`、`bin/update` | current tag 集合与已选通道；历史 baseline tag 与非 SemVer 形态仅作 fail-closed 反例 | canonical | `test_prism4_release_channel.py` | `uv run pytest tests/test_prism4_release_channel.py -q` | 发行单位改为非 tag 载体 |
