...
> 这不是安装文档，也不是完整架构文档。它只回答一个问题：**v3.0 beta 为什么要把 Prism 定位为轻量认知熵管理框架，并把 workflow 解释为认知熵治理工作流。**
> 当前口径：`v3.0.0-beta`（dogfood 验证期）。本页是叙事层说明，不新增受控术语；skill / CLI / validator 的具体演进走 decision gate，不在此页定义实现细节。
...
v3.0 beta 不改变 Prism 的 core contract：SDK + Vault Workspace + `uv` 仍是最小运行集合。变化在于 topic 内的状态治理方式更明确：
...
认知熵在 v3.0 beta 中是**设计哲学与叙事锚点**，不是新的 workflow 受控术语。
