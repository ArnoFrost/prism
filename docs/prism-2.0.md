...
Prism 2.0 把早期工作流管线收敛为一个更轻的本地优先工作系统：**治理框架可选**、**默认路径与内部状态痕迹解耦**、**workflow 复杂度刻意收敛**。
...
因此，Prism 2.0 更接近"一个**门槛更低、边界更清晰**的个人 AI 工作系统"，而不是功能堆叠。
...
v2.0 alpha 阶段把 5 件套全部落地：

| 件套 | 落点 | 含义 |
|------|------|------|
| **迁移指引** | `docs/migration.md` | 历史破坏性迁移入口；用户可感知变化、命令替换脚本、升级检查清单 |
...
> Prism 2.0 把早期双 alias / 治理硬入口 / 默认路径状态痕迹清偿掉了；v2.1 在此之上补齐了对外文档矩阵、受众分层和发布门禁。当前主要欠账在 dogfood 实证与对外试点节奏，而不在核心架构本身。
