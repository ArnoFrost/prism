...
> 当前阶段版本口径：`v2.0.0`。`v1.0.0` 与 `v1.1.x` 是已成立的历史里程碑；`v2.0` 表示在其之上完成的破坏性收敛与简化阶段。
...
- **v2.1 规划**：`docs/migration.md` / `audience` 元数据 / 机械守门 WARN / `docs/contributing.md` / 发布门禁约束
...
1. **继续 dogfood 验证**
   挑更多非自身 topic 跑完整 review，证明默认 lenient + 痕迹合并后体验确实更轻。

2. **推进 v2.1 治理**
   先修代码流程入口与默认面降噪，再推进 migration / contributing / 发布门禁。
