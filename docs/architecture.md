...
**Phase 6 — v1.0 验证** (v1.0.0-alpha)

- [x] 跨项目验证（TVKMM 完整跑通 13 轮评审 + 8 个决策，011 专项实证）
- [x] 触发 010 topic 周期性回顾（设计哲学 + 远期愿景校准，scope 11/13）
- [ ] 端到端新用户验证（干净环境 smoke test，含 SETUP.md 引导）— alpha 已知限制
- [ ] `yaml_get` 解析加固（技术债务）— 归入 beta
- [ ] Quick Topic 轻量入口（降低小任务进入门槛）— 归入 beta
