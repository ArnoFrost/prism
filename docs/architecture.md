...
> 本文档包含 Prism 的完整架构设计。首次使用请先阅读 [README](../README.md) 的快速开始；如果你想先看"Prism 现在是什么、为什么成立、还差什么"，请先读 [Prism 2.0 定位说明](./prism-2.0.md)。
...
v3.0 canary 在既有 workflow 基础上，把 Prism 的上层目标收敛为**长期人机协作中的认知熵治理**。这里的认知熵不是协议级术语，而是架构叙事锚点：复杂问题在时间维度上会出现理解发散、上下文遗忘、决策漂移、结构膨胀与重复重建，Prism 的工件与 skill 都是针对这些熵源的治理手段。
...
| 方向熵 | 不知道下一步该做什么 | `status` / `next`（候选能力） |
| 上下文熵 | 跨会话、跨设备恢复成本高 | handoff / digest / compact（按需） |
