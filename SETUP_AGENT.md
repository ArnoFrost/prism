# Prism Setup Guide（Agent 协议）

> **Agent SSOT** — 供 AI Agent 交互式引导用户完成 Prism 初始化。
> **人类自助安装** → [SETUP_GITHUB.md](SETUP_GITHUB.md)
> zip 安装用户经包内 INSTALL 跳转时，仍从 **Step 0** 开始（跳过 Step -1）。
...
```bash
bin/relink
# 或人类：./setup.sh init（含 relink + CLI 注入）
```
...
初始化完成后，日常命令面见 [docs/onboarding.md](docs/onboarding.md)（`bin/*` vs `prism <verb>` 分层 + E2E checklist）。
...
prism --version                   # 自查版本号（当前 v3.0.0-beta）
```
...
git checkout v<target>            # e.g. v3.0.0-beta（当前）/ v2.0.0-beta.1（v2 末版）/ v1.1.7（v1.x 末版）
cd "$HOME/prism-skills"
...
- 人类安装（GitHub）：[SETUP_GITHUB.md](SETUP_GITHUB.md)
- 兼容入口：[SETUP.md](SETUP.md)
