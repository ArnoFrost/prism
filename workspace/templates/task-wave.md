...
> **命名去冗余**：本文件叫 `wave-{M}_{slug}.md`——路径 `structures/task-{N}_{slug}/` 已是命名空间，不写 `task{N}-wave{M}.md`。
> **稳定 id 规则**：wave 编号只取数字 M；`slug` 只做人类可读，不参与身份判断。
...
反例（≥2，命名去冗余 + 单一决策链 + wave 归属）：
  ...
  ❌ 用 slug 替代 wave 编号 —— 编号 M 才是批次顺序，slug 只是可读补充
  ❌ 在 wave 里写决策 / 评审结论 —— wave 只推进执行；治理需求冒泡回 topic 根 reviews/decisions
  ❌ 无 task 的 topic 建 waves/ 目录 —— 该场景 wave 只在 focus 当前轮；长期 wave 信号 = 升级 structures/task-N_slug
