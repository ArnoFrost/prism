...
> **Prism 3.1 Lite development line** — 在 v3.0 GA 基线上启动强模型时代的治理瘦身：降低默认 workflow 表面积与 skill 热路径复杂度，同时保留 `scope` / `focus` / `review` / `decision` 的跨会话状态、审计与迁移价值。

### Changed

- **版本与公开口径** — `VERSION` / `pyproject.toml` / `uv.lock` 显性化为 `v3.1.0` / `3.1.0`，README 首屏标注 3.1 Lite development line。
- **发布叙事** — 明确 3.1 Lite 是治理瘦身路线启动，不宣称所有 3.1 行为面已经完成发布。
