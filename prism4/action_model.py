...
def canonical_action_model(plan: Artifact) -> dict[str, object]:
    ...
    for raw_line in plan.body.replace("\r\n", "\n").replace("\r", "\n").split("\n"):
        ...
        if heading:
            ...
            continue
        item = _LIST_RE.match(normalized)
        if item:
            # Preserve encounter order in the array, but not cosmetic bullet glyphs.
            text = _CHECKBOX_RE.sub("", item.group(1))
            output.append({"kind": "item", "text": _text(text)})
            continue
